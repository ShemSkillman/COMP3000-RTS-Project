...
namespace RTSEngine
{
    ...
    public class NPCUnitCreator : NPCComponent
    {
        ...
        [SerializeField, Tooltip("Units that this component is able to create other than attack units, builders and collectors.")]
        private List<Unit> independentUnits = new List<Unit>(); //units that can be created by this NPC component that do not include attack units, builders and resource collectors
        ...
        public NPCUnitRegulator GetActiveUnitRegulator(string code)
        {
            //if the active unit regulator instance for the sepififed building code exists, return it
            if (activeUnitRegulators.TryGetValue(code, out ActiveUnitRegulator aur))
                return aur.instance;
        ...
        public void DestroyActiveRegulator(string unitCode)
        {
            //if there's a unit regulator that manages the unit with the input code, disable it
            GetActiveUnitRegulator(unitCode)?.Disable();
